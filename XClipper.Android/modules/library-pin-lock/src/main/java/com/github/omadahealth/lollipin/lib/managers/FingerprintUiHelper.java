...
public class FingerprintUiHelper extends FingerprintManager.AuthenticationCallback {
    ...
    public void onAuthenticationSucceeded(FingerprintManager.AuthenticationResult result) {
        ...
        setImageResourceAttr(mIcon, R.attr.lp_finger_print_success_drawable);
        setTextColorAttr(mErrorTextView, R.attr.lp_finger_print_success_color);
    ...
    private void showError(CharSequence error) {
        setImageResourceAttr(mIcon, R.attr.lp_finger_print_error_drawable);
        setTextColorAttr(mErrorTextView, R.attr.lp_finger_print_error_color);
    ...
    Runnable mResetErrorTextRunnable = new Runnable() {
        @Override
        public void run() {
            setTextColorAttr(mErrorTextView, R.attr.lp_finger_print_color);
            mErrorTextView.setText(
    ...
    private static void setImageResourceAttr(ImageView view, @AttrRes int attr) {
        int resId = getAttributeRes(view.getContext(), attr);
        if (resId != -1) view.setImageResource(resId);
    }
    private static void setTextColorAttr(TextView view, @AttrRes int attr) {
        int resId = getAttributeRes(view.getContext(), attr);
            int data = getAttributeData(view.getContext(), attr);
            view.setTextColor(data);
    }

    private static int getAttributeRes(Context context, @AttrRes int attr) {
        TypedValue value = new TypedValue();
        context.getTheme().resolveAttribute(attr, value, true);
        return value.resourceId;
    }
    private static int getAttributeData(Context context, @AttrRes int attr) {
        TypedValue value = new TypedValue();
        context.getTheme().resolveAttribute(attr, value, true);
        return value.data;
    }
