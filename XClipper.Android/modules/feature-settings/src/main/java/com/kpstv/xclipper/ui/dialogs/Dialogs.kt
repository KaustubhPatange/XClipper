...
object Dialogs {
    ...
    fun showImproveDetectionDialog(context: Context, additional: CustomLottieDialog.() -> Unit = {}) {
        CustomLottieDialog(context)
            ...
            .setLoop(true)
            .setPositiveButton(R.string.adb_enable) { improveDetectionAdbDialog(context) }
    ...
    private fun improveDetectionAdbDialog(context: Context) {
        ...
        spannableString.append(context.getString(R.string.adb_detection_note))
        spannableString.append("\n\n")
        spannableString.append(context.getString(R.string.adb_detection_instruction, context.packageName), BackgroundColorSpan(color), Spanned.SPAN_INCLUSIVE_INCLUSIVE)
